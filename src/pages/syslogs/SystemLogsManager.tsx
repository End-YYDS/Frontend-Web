...
import type { GetSysLogsQueryRequest, GetSysLogsResponse, SysLogs } from './types';
...
export const SystemLogsManager = () => {
  const [syslogs, setSysLogs] = useState<SysLogs>({});
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        const { data } = await axios.get<GetSysLogsResponse>('/api/logs/sys');
    ...
    <div className='space-y-4'>
      ...
      <div className='flex space-x-4'>
        ...
        <Button style={{ backgroundColor: '#7B86AA' }} className='hover:opacity-90 text-white'>
          ...
          <span>Export {selectedLogs.length > 0 ? `(${selectedLogs.length})` : '(All)'}</span>
        </Button>
      </div>

      {/* Results count and applied filters */}
      <div className='flex items-center justify-between'>
        <span className='text-sm text-muted-foreground'>Selected: {selectedLogs.length}</span>
