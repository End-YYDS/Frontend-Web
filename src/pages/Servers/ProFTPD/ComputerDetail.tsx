...
export function ComputerDetail({ computerId, onBack }: ComputerDetailProps) {
  ...
  /** 取得 ProFTPD 狀態 */
  const fetchFtpStatus = useCallback(async () => {
    setLoading(true);
    try {
      ...
      toast.error('Error', { description: 'Failed to fetch ProFTPD status' });
    } finally {
      setLoading(false);
    }
  }, [computerId]);
  ...
  const performAction = async (action: 'start' | 'stop' | 'restart') => {
    ...
    try {
      ...
      if (data.Type === 'Ok') {
        toast.success('Success', { description: `ProFTPD ${action} succeeded` });
        fetchFtpStatus();
      } else {
        toast.error('Error', { description: `Failed to ${action} ProFTPD: ${data.Message}` });
      }
    } catch (error) {
      console.error(`${action} failed`, error);
      toast.error('Error', { description: `Failed to ${action} ProFTPD` });
    } finally {
      setActionLoading('');
    }
  };
        ...
        <Card>
          <CardContent className='p-4 flex items-center gap-3'>
            ...
            <div>
              ...
              <Badge className={ftpStatus.Status === 'active' ? 'bg-green-500' : 'bg-red-500'}>
        ...
        <CardContent>
          {ftpStatus.Sessions.length === 0 ? (
            ...
            ftpStatus.Sessions.map((s, i) => (
              <div key={i} className='border-b border-slate-200 py-2 flex justify-between text-sm'>
                <div>
        ...
        <CardContent>
          <Tabs defaultValue='errors' className='w-full'>
            <TabsList className='grid w-full grid-cols-2'>
              <TabsTrigger value='errors'>Error Logs ({ftpStatus.Logs.ErrLength})</TabsTrigger>
              <TabsTrigger value='access'>Access Logs ({ftpStatus.Logs.AccLength})</TabsTrigger>
            </TabsList>
