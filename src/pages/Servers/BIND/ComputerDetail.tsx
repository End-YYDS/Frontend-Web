...
export function ComputerDetail({ computerId, onBack }: ComputerDetailProps) {
  ...
  const fetchServerStatus = useCallback(async () => {
    setLoading(true);
    try {
      const sendData: GetBindRequest = { Uuid: computerId };
      const res = await axios.post<GetBindResponse>('/api/server/bind', sendData);
  ...
  /** 執行 BIND 操作 (Start / Stop / Restart) */
  const performAction = async (action: 'start' | 'stop' | 'restart') => {
    setActionLoading(action);
    try {
      const sendData: PostBindActionRequest = { Uuid: computerId };
      const res = await axios.post<PostBindActionResponse>(
        `/api/server/bind/action/${action}`,
        sendData,
      );
      const data = res.data;
        ...
        <Card>
          <CardContent className='p-12 text-center'>
            ...
            <h3 className='text-lg font-semibold text-slate-600 mb-2'>
              Failed to load BIND server status
            </h3>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div>
            <h1 className='text-2xl font-bold text-slate-800'>
              BIND DNS Server - {serverStatus.Hostname}
            </h1>
        ...
        <CardContent>
          <Tabs defaultValue='errors' className='w-full'>
            <TabsList className='grid w-full grid-cols-2'>
              ...
              <TabsTrigger value='queries'>Query Logs ({serverStatus.Logs.QryLength})</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value='queries' className='space-y-4'>
              {serverStatus.Logs.QueryLog.map((log, i) => (
                  ...
                  <CardContent className='p-4 flex justify-between'>
                    <div className='flex-1'>
                      ...
                      <p className='text-xs text-slate-600 mb-1'>
                        Client: {log.Client} | Response: {log.Response}
                      </p>
                      <p className='text-xs text-slate-600'>Duration: {log.Duration} ms</p>
                    </div>
                    <div className='text-right text-xs text-slate-500'>
                      <p>
                        {log.Date.Month} {log.Date.Day}, {log.Date.Year}
