...
export function ComputerDetail({ computerId, onBack }: ComputerDetailProps) {
  ...
  const performAction = async (action: 'start' | 'stop' | 'restart') => {
    ...
    try {
      ...
      if (data.Type === 'Ok') {
        toast.success('Success', { description: `Nginx ${action} succeeded` });
        fetchNginxStatus();
      } else {
        toast.success('Error', { description: data.Message || `Failed to ${action} Nginx` });
      }
    } catch (error) {
      console.error(`${action} failed`, error);
      toast.error('Error', { description: `Failed to ${action} Nginx` });
    } finally {
      setActionLoading('');
    }
  };
          ...
          <CardContent className='p-4 flex items-center gap-3'>
            ...
            <div>
              ...
              <Badge
                ...
                className={nginxStatus.Status === 'active' ? 'bg-green-500' : 'bg-red-500'}
              >
        ...
        <CardContent>
          <Tabs defaultValue='errors' className='w-full'>
            <TabsList className='grid w-full grid-cols-2'>
              <TabsTrigger value='errors'>Error Logs ({nginxStatus.Logs.ErrLength})</TabsTrigger>
              <TabsTrigger value='access'>Access Logs ({nginxStatus.Logs.AccLength})</TabsTrigger>
            </TabsList>
