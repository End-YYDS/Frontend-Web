...
export function BackupContent() {
  ...
  const fetchBackupHistoryApi = async () => {
    try {
      const { data } = await getBackupRoot({ body: { Limit: 5 }, throwOnError: true });
      const backups =
        data?.Backups?.map((b: InnerGetBackupResponse, idx: number) => ({
          id: `${idx}`,
          filename: b.Name,
  ...
  const backupNowApi = async (name: string, location: BackupLocation): Promise<Backup> => {
    const { data } = await postBackupRoot({
      body: { Type: location, Name: name },
      throwOnError: true,
    });
    if (!data || data.Type !== 'Ok') throw new Error(data?.Message ?? 'Backup failed');
  ...
  const restoreBackupApi = async (backup: Backup, location: BackupLocation) => {
    const sendData: ReductionRequest =
      location === 'Local'
        ? { Type: 'Local', file: backup.filename }
        : { Type: 'Remote', name: backup.filename };

    const { data } = await postReduction({ body: sendData, throwOnError: true });
    if (!data || data.Type !== 'Ok') throw new Error(data?.Message ?? 'Restore failed');
  };
