...
const SoftwarePackagesPage: PageComponent = () => {
  ...
  const installedPackages = selectedPcData
    ? Object.entries(selectedPcData.packages).filter(
        ([, pkg]) =>
          pkg.status === 'Installed' && pkg.name.toLowerCase().includes(searchTerm.toLowerCase()),
      )
    : [];
  ...
  const renderLoadMoreButton = () => {
    ...
    return (
      <div className='mt-4 text-center'>
        <Button variant='outline' onClick={handleLoadMore} disabled={isLoading} className='w-full'>
  ...
  return (
    <div className='container mx-auto py-6 px-4'>
      <div className='bg-[#A8AEBD] py-1.5 mb-6'>
        <h1 className='text-4xl font-extrabold text-center text-[#E6E6E6]'>Software Packages</h1>
      </div>
            ...
            <CardContent>
              {isMobile ? (
                  ...
                  {currentPackages.length === 0 ? (
                    <div className='text-center py-8 text-muted-foreground'>
                      {isLoading
                        ? 'Loading...'
                        : searchTerm
                        ? 'No matching installed packages found'
                ...
                <Table>
                  <TableCaption>
                    {searchTerm
                      ? `Search results: ${installedPackages.length} installed packages`
                      : `Total ${installedPackages.length} installed packages`}
                  </TableCaption>
                  <TableHeader>
                    <TableRow>
      ...
      <AlertDialog open={showUninstallDialog} onOpenChange={setShowUninstallDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to remove <strong>{packageToUninstall?.name}</strong>. This action will
              uninstall the package, but you can reinstall it later.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
