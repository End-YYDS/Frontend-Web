...
const ModuleManagementTab = () => {
    ...
    try {
      const data = [
        { 
          ...
          configOptions: [
            { name: "interval", type: "number", label: "監控間隔(秒)", defaultValue: 30, description: "設定監控資料收集間隔" },
            { name: "alerts", type: "boolean", label: "啟用警告", defaultValue: true },
            { name: "logLevel", type: "select", label: "日誌等級", options: ["DEBUG", "INFO", "WARN", "ERROR"], defaultValue: "INFO" }
          ],
        ...
        { 
          ...
          configOptions: [
            ...
            { name: "format", type: "select", label: "日誌格式", options: ["JSON", "TEXT", "XML"], defaultValue: "JSON" }
          ],
  ...
  const handleSaveModuleSettings = async (moduleId: string, settings: Record<string, ConfigValue>) => {
    try {
                ...
                modules.map((module) => (
                  <React.Fragment key={module.id}>
                    ...
                    {expandedModules.has(module.id) && (
                      <ModuleConfigPanel module={module} />
                    )}
                  </React.Fragment>
                ))
              )}
