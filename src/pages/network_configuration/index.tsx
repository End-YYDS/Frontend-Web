...
const NetworkConfigurationPage: PageComponent = () => {
  ...
  const [routeToDelete, setRouteToDelete] = useState<{ id: string; destination: string } | null>(
    null,
  );
  ...
  const handleSaveInterface = async (interfaceData: Partial<NetworkInterface>) => {
    ...
    try {
      ...
      // 更新前端 state
      setInterfaces((prev) =>
        prev.map((iface) => (iface.id === Nid ? { ...iface, ...interfaceData } : iface)),
      );
  ...
  const handleToggleInterface = async (interfaceId: string) => {
    try {
      ...
      const actionUrl =
        iface.status === 'Up' ? `/api/network/net/action/down` : `/api/network/net/action/up`;
      ...
      if (res.data.Type === 'OK') {
        ...
        setInterfaces((prev) =>
          prev.map((i) =>
            i.id === interfaceId ? { ...i, status: i.status === 'Up' ? 'Down' : 'Up' } : i,
          ),
        );

        toast.success(
          `Network interface ${iface.name} ${
            iface.status === 'Up' ? 'stopped' : 'started'
          } successfully`,
        );
      } else {
        toast.error(
          `Failed to ${iface.status === 'Up' ? 'stop' : 'start'} interface: ${res.data.Message}`,
        );
      }
    } catch (error) {
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {interfaces
                      .filter((iface) => iface.type === 'virtual')
                      .map((iface) => (
                        <TableRow key={iface.id}>
                          <TableCell className='font-medium'>{iface.name}</TableCell>
                          <TableCell>{iface.ipv4}</TableCell>
              ...
              <CardContent className='space-y-4'>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setDnsConfig((prev) => ({ ...prev, hostname: e.target.value }))
                    }
      ...
      <AlertDialog open={showDeleteRouteDialog} onOpenChange={setShowDeleteRouteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to delete the route with destination{' '}
              <strong>{routeToDelete?.destination}</strong>. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
                  ...
                  <FormControl>
                    ...
                    >
                      <div className='flex items-center space-x-2 flex-1'>
                        ...
                        <Label htmlFor='dhcp' className='cursor-pointer'>
                          Automatic (DHCP)
                        </Label>
                      </div>
                      <div className='flex items-center space-x-2 flex-1'>
